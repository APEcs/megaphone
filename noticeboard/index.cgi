...
my $template = Template -> new(basedir   => path_join($settings -> {"config"} -> {"base"}, "templates"),
                               langdir   => path_join($settings -> {"config"} -> {"base"}, "lang"),
                               blockname => 1,
...
$pageblock = $settings -> {"config"} -> {"NBoard::default_block"} if(!$pageblock); # This ensures $pageblock is defined and non-zero
